...
public class HealthDataServiceImpl implements IHealthDataService {
    ...
    public List<Map<String, Object>> getHealthDataByToday(LocalDate date) {
        ...
        List<StepCount> stepCountList = stepCountMapper.selectList(new QueryWrapper<StepCount>()
                .eq("date", date)
                .eq("user_id", userId));

        // 查询SleepAnalysis表中指定日期和用户的数据
        List<SleepAnalysis> sleepAnalysisList = sleepAnalysisMapper.selectList(new QueryWrapper<SleepAnalysis>()
                .eq("date", date)
                .eq("user_id", userId));

        // 查询HeartRate表中指定日期和用户的数据
        List<HeartRate> heartRateList = heartRateMapper.selectList(new QueryWrapper<HeartRate>()
                .eq("date", date)
                .eq("user_id", userId));
        ...
        for (StepCount stepCount : stepCountList) {
            ...
            if (!maxStepCountMap.containsKey(currentDate) ||
                    stepCount.getValue() > maxStepCountMap.get(currentDate).getValue()) {
        ...
        for (SleepAnalysis sleepAnalysis : sleepAnalysisList) {
            ...
            if (!maxSleepAnalysisMap.containsKey(currentDate) ||
                    sleepAnalysis.getValue() > maxSleepAnalysisMap.get(currentDate).getValue()) {
        ...
        for (HeartRate heartRate : heartRateList) {
            ...
            if (!maxHeartRateMap.containsKey(currentDate) ||
                    heartRate.getValue() > maxHeartRateMap.get(currentDate).getValue()) {
